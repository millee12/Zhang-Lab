...
!
        ...
        do inl=1,nen
            ...
            if ((x(1,inl)>=12.7) .and. (x(1,inl)<=XBmin) .and. (x(2,inl)>=1.391) .and. (flagnearimg(node)==1)) then
                flagclus(inl) = 1
            endif
            if ((x(1,inl)<=15.0) .and. (x(1,inl)>=XBmax) .and. (x(2,inl)>=1.391) .and. (flagnearimg(node)==1)) then
                flagclds(inl) = 1
            endif
            fnode(:,inl)=0.0
            d(1:ndf,inl) =  dloc(1:ndf,ien(inl,ie))
